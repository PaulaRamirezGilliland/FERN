...
class RigidTransform(object):
    ...
    def clone(self):
        if self._axisangle is not None:
            return RigidTransform(self._axisangle.clone(), self.trans_first)
        else:
            return RigidTransform(self._matrix.clone(), self.trans_first)
...
def random_angle(n, restricted, device):
    ...
    # Standard view sampling (through-plane rotation)
    if restricted:
        a = 2*np.pi*np.random.rand(n)
        # Sample from normal dist centered at zero (standard views)
        b = np.random.normal(0, (np.pi/4)/3, n)
        b[b>np.pi/4] = 0
        b[b<-np.pi/4] = 0

    # Sampling as in SVoRT (through-plane rotation)
    else:
        a = 2 * np.pi * np.random.rand(n)
